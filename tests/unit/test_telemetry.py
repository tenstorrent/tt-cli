...
def test_telemetry_id_command_prints_the_distinct_id(runner, collected):
    ...
    assert shown == {"distinct_id": TelemetryState(get_paths()).instance_id()}
    assert all(event["distinct_id"] == shown["distinct_id"] for event in collected)
    plain = runner.invoke(app, ["self", "telemetry-id"])
    assert plain.stdout.strip() == shown["distinct_id"]
